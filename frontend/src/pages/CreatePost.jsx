...
const CreatePost = () => {
  ...
  const handleContentChange = useCallback(() => {
    if (editorRef.current) {
      ...
      // Check active formats for button states
      checkActiveFormats();
    }
  }, [banner, checkActiveFormats]);
  ...
  const executeCommand = useCallback(
    (command, value = null) => {
      ...
      // Enable style with CSS for better formatting support
      ...
      // For color commands, we don't need to reset to white
      if (command === "foreColor" || command === "hiliteColor") {
        return;
      }

      // For other commands, ensure text remains white if no specific color was set
      setTimeout(() => {
        if (editorRef.current && range) {
          selection.removeAllRanges();
          selection.addRange(range);

          // Only reset color if it's not already set to something specific
  ...
  const uploadImageToServer = async (file) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/upload/posts/single`,
        formData,
        {
          headers: {
  ...
  const fetchPostForEdit = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/posts/${editPostId}/edit`,
        {
  ...
  const handleKeyDown = useCallback(
    (e) => {
      ...
      setTimeout(() => {
        ...
        // Ensure white text color is maintained
        const selection = window.getSelection();
    ...
    [executeCommand, checkActiveFormats]
  );
  ...
  const ToolbarButton = ({
    ...
    const isActive = command ? activeFormats.has(command) : active;
  ...
  const handlePublishPost = async () => {
    ...
    try {
      ...
      showToast.success(
        res.data.message || (isEditing ? "Post updated!" : "Post published!")
      );
      navigate("/");
    } catch (err) {
      ...
      {banner && (
        <div className="mb-3 sm:mb-4 md:mb-6 border border-white rounded-lg overflow-hidden">
          ...
          <img
            src={banner}
            alt="Banner"
            className="w-full h-24 sm:h-32 md:h-40 lg:h-48 xl:h-56 object-cover"
          />
        </div>
      )}
